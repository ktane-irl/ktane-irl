<template>
    <v-card class="mx-auto" max-width="600" :loading="is_sending || is_fetching" :disabled="is_sending">
        <v-col>
            <v-row>
            ...
            <v-card-text>
                <div v-for="(item, i) in config?.wiresColor" :key="i">
...
export default Vue.extend({
    ...
    data: () => ({
        ...
        config: undefined as WiresConfiguration | undefined,
        wireColor
    }),
    methods: {
        ...
        fetch_config() {
            client.get(`/api/Wires/1/${this.current_slot}`)
                .then((response) => {
        ...
        send_config() {
            if (this.config) {
                ...
                client.put(`/api/Wires/1/${this.current_slot}`, {
